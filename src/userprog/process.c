...
#include "threads/synch.h"     /* semaphore implementation */
...
{
   struct plist_elem* p = plist_find(thread_current()->tid);
   if(p != NULL){
      lock_acquire(&p->lock); // Acquire lock for this process
      
      p->exit_status = status;

      lock_release(&p->lock); // Release lock for this process

      plist_set_parent_dead(thread_current()->tid); // Mark parent as dead for all children
   }
}

//Add print of the name of the process
void plist_print_format(struct plist_elem* p)
{
   if(p->pid != 0)
      printf("%i\t%i\t\t%i\t\t%d\t%d\n", p->pid, p->parent_id, p->exit_status, p->alive, p->parent_alive);
}
...
{
   printf("-------------------------------------\n");
   printf("id\tparent-id\texit-status\talive\tparent-alive\n");
   plist_for_each(plist_print_format);
   printf("-------------------------------------\n");
}
...
process_execute (const char *command_line)
{
  ...
  debug("%s#%d: process_execute(\"%s\") ENTERED\n",
   ...
   arguments.parent_id = thread_current()->tid; // Setting parentid, so to remember it in process_start
   ...
   sema_init(&arguments.sema, 0); // Initialize semaphore for synchronization
  ...
  // Free the command line memory - it's safe now since start_process has finished loading
  free(arguments.command_line);
...
start_process (struct parameters_to_start_process* parameters)
{
   ...
   //flist_init(&thread_current()->flist);
  ...
  if (success)
  {
      ...
      plist_insert(thread_current()->tid, parameters->parent_id); // Insert process in process list
  }
   ...
   /* Signal parent that loading is complete (success or failure) */
...
process_wait (int child_id)
{
  ...
  struct plist_elem* child = plist_find(child_id);

  debug("%s#%d: process_wait(%d) ENTERED\n",
        cur->name, cur->tid, child_id);

   /* Validate the child process */
   if (child == NULL || child->parent_id != cur->tid || child->waited) {
      return -1;
   }

   /* Wait for child to exit if it's still running */
   if (child->alive) {
      sema_down(&child->sema);
   }

   /* Mark as waited and get status */
   lock_acquire(&child->lock);
   child->waited = true;
   status = child->exit_status;
   lock_release(&child->lock);
   // /* Clean up the child if it can be removed now */
   // if (!child->alive && child->waited) {
...
process_cleanup (void)
{
  ...
  struct plist_elem* p = plist_find(cur->tid);
  int status = -1;
   ...
   */
   ...
   /* Clean up file list if initialized */
   ...
   /* Get exit status and perform process list management */
   if(p != NULL) {
      //status = p->exit_status;
      
      lock_acquire(&p->lock); // Acquire lock for this process
      p->alive = false;   
      sema_up(&p->sema); // Signal any waiting parent
      lock_release(&p->lock); // Release lock for this process

      /* If process can be removed (dead and waited or orphaned), do it now */
      if (!p->alive && (p->waited || !p->parent_alive)) {
         plist_remove(p->pid);
      }

      // /* If process can be removed (dead and waited or orphaned), do it now */
      // if (!p->alive && (p->waited || !p->parent_alive) && p->parent_id != 1) { // Added parent id > 1 to not remove child process of the kernel thread
      //    plist_remove(p->pid); // Remove from plist
      // }
   }
   
   /* Clean up any orphaned processes */
   plist_clean_everything();

   /* Print the process exit status - keep this format for tests */
